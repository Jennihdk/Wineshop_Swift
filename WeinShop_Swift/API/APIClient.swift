...
struct APIClient {
    ...
    let urlString = "https://public.syntax-institut.de/apps/batch1/JenniferHedtke/data.json"
    ...
    func getWineList(completion: @escaping([Wine]) -> Void) {
        ...
        //URL Session und Datatask
        let dataTask = URLSession.shared.dataTask(with: url!) { data, response, error in
            if error == nil && data != nil {
                let decoder = JSONDecoder()
                
                do {
                    let wineList = try decoder.decode([Wine].self, from: data!)
                    completion(wineList)
                } catch {
